...
public class ValidationMethods {
	...
	public static int getValidInt(Scanner sIn, String question, String warning, int[] range){
		...
		do {
			...
			try {
				intValue= Integer.parseInt(input);//validating an integer value
				if (range.length==0) {//checking for range
					break;
				} else if (range.length==2) {
					 if(intValue>=range[0] && intValue<=range[1]) {//within range
	                        break;
	...
	public static double getValidDouble(Scanner sIn, String question, String warning, double[] range){
		...
		do {       
			...
			try {
				dValue = Double.parseDouble(input);//checking for double value
				if(range.length==0) {//checking for range
					break;
				} else if (range.length==2) {
					if(dValue>=range[0] && dValue<=range[1]) {//within range
						break;
					}else {//not within range
						System.out.println(warning);
					}
	...
	public static String getValidString(Scanner sIn, String question, String warning, String[] choices){
        ...
        do {       
           ...
           if (choices.length==0) {//checking for range
                validInput=true;
            } else {//within range
            for (int i=0;i<choices.length;i++) {
                if (choices[i].toUpperCase().equals(sValue.toUpperCase())) {
                    validInput=true;
                    break;
                }
            }        
            }
            if (!validInput) {//not within range
                System.out.println(warning);                    
            }
        } while (!validInput);        
        return sValue;        
